...
import gov.grants.apply.services.applicantwebservices_v2.GetOpportunitiesPlusCompTitleResponse.OpportunityInfo;
...
public class GetOpportunitiesPlusCompTitleClient extends BaseApplicantClient {

	private static final String CLASSNAME = GetOpportunitiesPlusCompTitleClient.class.getSimpleName();
	...
	public static void main( String[] args ) {
		...
		try {
			GetOpportunitiesPlusCompTitleClient client = new GetOpportunitiesPlusCompTitleClient();
	...
	public void makeServiceCall() throws Exception {
		try {
			...
			GetOpportunitiesPlusCompTitleRequest request = new GetOpportunitiesPlusCompTitleRequest();
			
			request.setFundingOpportunityNumber( oppId );
			request.setCFDANumber( cfda );
			request.setCompetitionID( compId );
			
			GetOpportunitiesPlusCompTitleResponse response = port.getOpportunitiesPlusCompTitle(request);
			...
			if( oppInfoList != null && oppInfoList.size() > 0) {	
				...
				for ( OpportunityInfo info : oppInfoList ) {
					...
					sb.append( "\nCompetition Title: " ).append( info.getCompetitionTitle() );
